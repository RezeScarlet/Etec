						...
						<input type="submit" value="Enviar" class="btn-md btn m-2 text-white" style="background: #483fff;">
							...
							<?php
								...
								if ($_POST) {
									...
									$sql = "select cod_produto from produtos WHERE cod_produto = '$codigo'";
									$result=mysqli_query($conexao, $sql);
									$numrows = mysqli_num_rows($result);

									if ($numrows > 0) {
										$sqlDelete = "DELETE FROM produtos where cod_produto = '$codigo'";
										$resultDelete = mysqli_query($conexao, $sqlDelete);

										if ($resultDelete) {
											echo "<p class='alert alert-success'>Produto $codigo apagado com sucesso.</p>";
										} else {
											echo "<p class='alert alert-danger'>Um erro ocorreu.</p>";
										}

									} else {
										echo "<p class='alert alert-warning'>Este produto não existe.</p>";
									}
